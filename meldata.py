...
# Updated MelSpectrogram initialization with sample_rate parameter
to_mel = torchaudio.transforms.MelSpectrogram(
    sample_rate=24000,  # Added explicit sample_rate
    n_mels=80, 
    n_fft=2048, 
    win_length=1200, 
    hop_length=300
...
class FilePathDataset(torch.utils.data.Dataset):
    def __init__(self,
        ...
        # Updated MelSpectrogram with explicit sample_rate
    ...
    def _load_tensor(self, data):
        ...
        # Resample if necessary using librosa with updated parameters
        if sr != self.sr:
            wave = librosa.resample(wave, orig_sr=sr, target_sr=self.sr)
            print(f"Resampled {wave_path} from {sr} to {self.sr}")
...
class Collater(object):
    ...
    Args:
      return_wave (bool): whether to return wave data
    """
    ...
    def __call__(self, batch):
        ...
        ref_texts = torch.zeros(batch_size, max_rtext_length, dtype=torch.long)

        input_lengths = torch.zeros(batch_size, dtype=torch.long)
        ref_lengths = torch.zeros(batch_size, dtype=torch.long)
        ...
        ref_labels = torch.zeros(batch_size, dtype=torch.long)
...
def build_dataloader(path_list,
    ...
    data_loader = DataLoader(
        ...
        persistent_workers=(num_workers > 0)  # Added for better performance
    )
